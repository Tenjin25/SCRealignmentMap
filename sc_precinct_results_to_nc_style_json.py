...
out_json = r'C:\Users\Shama\OneDrive\Documents\Course_Materials\CPT-236\Side_Projects\SCRealignments\sc_precinct_results_structured.json'
...
def get_contest_type(contest_name):
    ...
    if "GOVERNOR" in name and "LIEUTENANT" not in name:
        return "governor"
    elif "LIEUTENANT GOVERNOR" in name:
        return "lt_governor"
    elif "U.S. SENATE" in name or "SENATE" in name:
        return "us_senate"
    elif "PRESIDENT" in name:
        return "presidential"
...
for year_str in sorted(year_csv_map.keys()):
    ...
    # Aggregate all contests found in the data
    all_contests = results_df['office'].dropna().unique()
    print(f"Found contests: {list(all_contests)}")
    ...
    for contest_name in all_contests:
        ...
        actual_df = results_df[results_df['office'].str.upper() == contest_name.upper()]
        if actual_df.empty:
    ...
    print(f"Year {year_str}: processed {contests_processed} contests")

print(f"\n=== SUMMARY ===")
total_contests = sum(len(contests) for contests in output.values())
print(f"Total years processed: {len(output)}")
print(f"Total contests processed: {total_contests}")
...
print(f'NC-style JSON saved to {out_json}')
